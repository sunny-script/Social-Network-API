...
const seedDatabase = async () => {
    try {
        ...
        await User.findOneAndUpdate({ username: 'john_doe' }, { $push: { thoughts: [thoughts[0]._id, thoughts[1]._id] } });
        await User.findOneAndUpdate({ username: 'jane_doe' }, { $push: { thoughts: [thoughts[2]._id] } });
        await User.findOneAndUpdate({ username: 'sam_smith' }, { $push: { thoughts: [thoughts[3]._id] } });
        await User.findOneAndUpdate({ username: 'lucy_liu' }, { $push: { thoughts: [thoughts[4]._id] } });
        ...
        await User.findByIdAndUpdate(users[0]._id, // John
        { $addToSet: { friends: [users[1]._id, users[2]._id] } });
        await User.findByIdAndUpdate(users[1]._id, // Jane
        { $addToSet: { friends: [users[0]._id, users[3]._id] } });
        await User.findByIdAndUpdate(users[2]._id, // Sam
        { $addToSet: { friends: [users[0]._id, users[3]._id] } });
        await User.findByIdAndUpdate(users[3]._id, // Lucy
        { $addToSet: { friends: [users[1]._id, users[2]._id] } });
