...
export const addFriend = async (req, res) => {
    try {
        ...
        const user = await User.findByIdAndUpdate(userId, { $addToSet: { friends: friendId } }, // Prevent duplicates
        { new: true });
        // Add user to friend's friends list
        const friend = await User.findByIdAndUpdate(friendId, { $addToSet: { friends: userId } }, // Prevent duplicates
        { new: true });
        if (!user || !friend) {
...
export const removeFriend = async (req, res) => {
    try {
        ...
        const user = await User.findByIdAndUpdate(userId, { $pull: { friends: friendId } }, // Remove friendId from friends
        { new: true });
        // Remove user from friend's friends list
        const friend = await User.findByIdAndUpdate(friendId, { $pull: { friends: userId } }, // Remove userId from friends
        { new: true });
        if (!user || !friend) {
            res.status(404).json({ message: 'User or friend not found' });
            return;
